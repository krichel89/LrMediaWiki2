...
-- Every shipped build that is actually present, in the order we want to try
-- them. We deliberately do NOT detect the CPU architecture: there is no
-- documented SDK call for it, and shelling out to `uname -m` would add one
-- more unverifiable moving part. On a Mac the native arm64 build is tried
-- first; on an Intel Mac it cannot start, no port file appears, and start()
-- moves on to the x86_64 build. That costs a few seconds once on Intel and
-- can never pick a binary the machine is unable to run.
local function binaryCandidates()
	...
	if WIN_ENV == true then
		...
		names = { 'sdcbridge-mac-arm64', 'sdcbridge-mac-x86_64' }
	end
	local found = {}
